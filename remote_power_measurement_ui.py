...
class ControllerGui(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        self.lineEdit_power.textChanged.connect(self.ui_utils.find_degree(float(self.lineEdit_power.text())))
...
class ControllerUtils():
    ...
    def run_calibration(self):
      ...
      #  self.gui.ui_utils.find_degree(data)
        data = data 
...
class UIUtils():
    ...
    def moveToPosition(self):
        ...
        motor1.moveToAbsolutePosition(motor=1, pos= bernd)
    ...
    def calibration(self, step_size=5., integration_time=1.):
        ...
        #####  for artificial random value generation   ######
        # a=0
        # art_measurement=[]
        # art_degrees=np.arange(0,360,step_size)
        # #art_measurement = np.arange(0,360,step_size)
        # art_measurement = np.random.normal(size=len(art_degrees))
        
        # index = np.where(art_measurement == self.find_nearest(art_measurement, float(self.gui.lineEdit_power.text())))
        # variable = (np.concatenate(index).tolist()) 
        # variable2 = variable[0]   
        # print("wanted value of", self.gui.lineEdit_power.text(), "was rounded to ", art_measurement[variable2]  ,"which has the index" , variable2)
        # corresponding_degree = art_degrees[variable2]
    ...
    def find_degree(self, powervalue):
        data = self.int_values
        print(data, type(data))
        index = np.where(data[:,1] == self.find_nearest(data[:,1], powervalue))
        variable = (np.concatenate(index).tolist()) 
        variable2 = variable[0]   
        print("wanted value of", self.gui.lineEdit_power.text(), "was rounded to ", data[:,1][variable2]  ,"which has the index" , variable2)
        corresponding_degree = data[:,0][variable2]
        print("the corresponding degree is", corresponding_degree)
        self.gui.lineEdit_degree.setText(str(corresponding_degree))
