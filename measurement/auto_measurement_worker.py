...
import shutil
shutil.rmtree(
    "N:\\data\\2016\\magnetometer\\cell66\\remote\\meas0",
    ignore_errors=True)
...
class MagnetometerWorker(object):
    configure_folder_path = None
    configure_filenames = None

    def __init__(self):
        self.configure_folder_path = os.path.join(os.getcwd(), 'configs')
        self.configure_filenames = os.listdir(self.configure_folder_path)

    def run_measurement(self):

        for config_filename in self.configure_filenames:
            config_filepath = os.path.join(self.configure_folder_path, config_filename)
            try:
            ...
            try:
                ...
                print("Using nidaq channel: "+str(nidaq.channel))
