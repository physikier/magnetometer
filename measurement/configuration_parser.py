...
#
def load_config(filepath, allowed_outputs=['B0', 'B1', 'R2', 'R3', 'R4', 'lock_in', 'motor']):
    ...
    def _check_integrity(config):
        ...
        if any([(output not in allowed_outputs) for output in all_outputs.keys()]):
            #print "all_outputs_keys:", all_outputs.keys()
            #print 'allowed_outputs:', allowed_outputs
            #print 'result:', [( output, bool(output not in allowed_outputs)) for output in all_outputs.keys()]
