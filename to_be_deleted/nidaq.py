...
DAQmx_Val_Diff                    = ctypes.c_int32(10106)
DAQmx_Val_NRSE                    = ctypes.c_int32(10078)
DAQmx_Val_PseudoDiff              = ctypes.c_int32(12529)
...
class CounterBoard:
    ...
    def startCounter(self, SettlingTime, CountTime):
        if self.CountLength() != numpy.inf:
            self.setCountLength(numpy.inf,1,1)
        self.setTiming(SettlingTime, CountTime)
        self.StartCI()
        self.StartCO()
...
'''
class NICounter(CounterBoard):
    def __init(self, CounterIn, CounterOut, TickSource)
        CounterBoard.__init__(self, CounterIn=CounterIn,
                                  CounterOut=CounterOut,
                                  TickSource=TickSource,
                                  )
        
    
    def getData(samples_per_sec,refreshs_per_sec=None)
        self.samplelength=int(refreshs_per_sec/samples_per_sec)
        
        if samplelength <1.:
            print 'refreshrate must be bigger than sample rate!!!!'
            break
        if round(samplelength) is not float(samplelength):
            print "warning: refreshrate and sample rate are not a of each other multiple"
        self. Set
        self.StartCI
'''    
...
class AITask(object):
    ...
    def __init__(self, Channels, N, f, read_timeout=1.0, range=(-10, 10)):
        ...
        self.Task = ctypes.c_ulong()
        CHK( dll.DAQmxCreateTask('', ctypes.byref(self.Task)) )
        CHK( dll.DAQmxCreateAIVoltageChan(self.Task, self.Channels, '', DAQmx_Val_Diff, ctypes.c_double(self.range[0]), ctypes.c_double(self.range[1]), DAQmx_Val_Volts, ''))
        CHK( dll.DAQmxCfgSampClkTiming(self.Task, self.clock_source, ctypes.c_double(f), DAQmx_Val_Rising, DAQmx_Val_ContSamps, ctypes.c_ulonglong(N)) )
...
class AITask_mod(object):
    ...
    def __init__(self, Channels, N, f, read_timeout=1.0, range=(-10, 10)):
        ...
        self.timeout = ctypes.c_double(4.*N/f)
        self.Nread = ctypes.c_long()
        self.data    = numpy.zeros((1,N), dtype=numpy.double )
        self.Task = ctypes.c_ulong()
        CHK( dll.DAQmxCreateTask('', ctypes.byref(self.Task)) )
        CHK( dll.DAQmxCreateAIVoltageChan(self.Task, self.Channels, '', DAQmx_Val_Diff, ctypes.c_double(self.range[0]), ctypes.c_double(self.range[1]), DAQmx_Val_Volts, ''))
        ...
        #CHK( dll.DAQmxCfgImplicitTiming(self.Task,DAQmx_Val_FiniteSamps,ctypes.c_uint64(self.N)))
...
class Thermistor(AITask):
    #IP 129.69.46.203 MAC:00:80:2F:12:BE:8A
    def __init__(self, Channels="/ENET-1596532/ai0:1", N=1, f=1, read_timeout=2.0, range=(-0.08, 0.08), R_ref=10000.):
        AITask.__init__(self, Channels, N, f, read_timeout, range)
        self.R_ref=R_ref
        try:
            self.Start()
        except:
    ...
    def get_R_T(self):
        ...
        return self.R_ref*(numpy.mean(voltages[0])/numpy.mean(voltages[1])-1.)
    def get_T(self):
        Res_T = self.get_R_T()
        T=(1./(1./298.15+1./3988.*numpy.log(Res_T/self.R_ref)))
        return T

class ThermoCouple(AITask):
    #IP 129.69.46.203 MAC:00:80:2F:12:BE:8A
    def __init__(self, Channels="/ENET-1596532/ai0:2", N=1, f=1, read_timeout=2.0, range=(-0.08, 0.08)):
        AITask.__init__(self, Channels, N, f, read_timeout, range)
        try:
            self.Start()
        except:
    ...
    def get_T(self):
        ...
        for i,v in enumerate(V):
            if v>=0:	
                T[i]= v *((2.592800E-2)+v*((-7.602961E-7)+v*((4.637791E-11)+v*((-2.165394E-15) +v*((6.048144E-20) +v*(-7.293422E-25))))));
            else:
                T[i]= v *((2.5949192E-2)+v*((-2.1316967E-7)+v*((7.9018692E-10)+v*((4.2527777E-13)+v*((1.3304473E-16)+v*((2.0241446E-20)+v*(1.2668171E-24)))))));
        self.Stop()
        return T
