...
cell='4'
measno='31'
measurement='rb85_50degC_less_than_15uW_40dB_R0asB0_R1asB1_withHighPass_R3R4R2Hameg' 		#number and special features of the measurement
...
ui=np.zeros([volR2steps, 2])
SetupTask()
plt.clf()
for j in range(volR2steps):
    FrequencyArray=np.zeros([numberPointsComp, fsteps])
    FrequencyArrayFft=np.zeros([numberPointsComp/2+1, fsteps])
    volR2=volR2start+j*volR2step
    hameg.set_voltage(volR2,ch=3)
    time.sleep(0.1)
    ui[j,0]=hameg.get_voltage(ch=3)
    ui[j,1]=hameg.get_current(ch=3)
    ...
    np.savetxt(filename+"/R2_{0:.3f}V.csv".format(volR2), FrequencyArray, delimiter=",")
    np.savetxt(filename+"/FFT_R2_{0:.3f}V.csv".format(volR2), FrequencyArrayFft, delimiter=",")

    #plt.yscale('log')
    plt.plot(FrequencyArray)
    plt.savefig(filename+'/R2_{0:.3f}V.png'.format(volR2))
    plt.clf()
    plt.plot(FrequencyArrayFft)
    plt.ylim(ymax=6, ymin=0)
    plt.savefig(filename+'/FFT_R2_{0:.3f}V.png'.format(volR2))
    plt.clf()

np.savetxt(filename+"/uiR2.csv", ui, delimiter=",")
...
params=[(measno, measurement), ("StartFrequency(Hz)", fstart), ("StopFrequency(Hz)", fstop),
 ...
 ("DownsamplingFactor", factor), ("B0Offset(V)", b0offsetstart), ("R3(V)", volR3start),
  ("R4(V)", volR4start), ("R2Start(V)", volR2start),
  ("R2Stop(V)", volR2stop), ("R2Step(V)", volR2step), ("B1Amplitude(V)", b1amp)]
